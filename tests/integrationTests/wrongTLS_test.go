...
func TestNATSConnection_WrongTLSCertPath_ReturnsConnectError(t *testing.T) {
	if testing.Short() {
	...
	options := []nats.Option{
		...
		natsutil.CAPemAuth("/tmp/definitely-missing-ca.pem"),
	...
	if err == nil {
		_ = nc.Drain()
		nc.Close()
		t.Fatal("expected connection error with wrong TLS cert path, got nil")
	}
}
